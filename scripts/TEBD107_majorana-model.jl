...
Ls = ( 2 .^ (3:lgLmax) ) .+ 1
maxdims = 2 .^ (1:lgmaxdimmax)
dts = [0.0625, 0.125]

dumb_time_estimate(d) = d[:L]*d[:maxdim]^3/d[:dt]
variable_param_list = [@dict L maxdim dt for L = Ls, maxdim = maxdims, dt = dts] |> arr1d
...
for vparam in variable_param_list
    dmt_params = Dict(:maxdim => vparam[:maxdim])
    delete!(vparam, maxdim)
    params = Dict((@dict dmt_params trotter jobname subdate T U commit init) ∪ vparam)
    run_te(params)
end
